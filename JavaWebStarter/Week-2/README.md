...
1. 필드(Field) : 필드는 클래스 객체의 상태 속성을 나타내며, 클래스 멤버 변수라고 불린다. 
 ...
 stack메모리는 메소드의 동작이 끝나면 사라집니다. heap메모리는 객체참조가 없으면 GC(Garbage Collector)가 적정 시점에 알아서 데이터를 정리해줍니다.
...
안드로이드 등 다른 프로그램 소스는 필드 변수명에 '_'(1글자) prefix를 사용해서 'this.'(5글자) 보다 효과적으로 사용하지만 자바기반 웹프로그래밍에서는 this.를 쓰는것이 자연스럽습니다.
